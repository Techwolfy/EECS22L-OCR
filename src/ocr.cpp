...
//Include reference image resource
#include "couriernew.pixbuf"

//Image Constants
const int width = 30, height = 56;
...
std::vector<Image> OCR::cropCharImages(Image input) {
	  ...
	  for(int i = 0; i < input.getHeight(); i += height) {
		    for(int j = 0; j < input.getWidth(); j += width) {
			      Image charImage(width, height);
