...
std::string OCR::recognize() {
	std::string text;
	std::vector<Image> img = charCrop(image);
	int newLine = 0;

        Image 
iCourierFont(Gdk::Pixbuf::create_from_file("CourierNew_bw.pixbuf"));
        std::vector<Image> iList = charCrop(iCourierFont);
...
std::vector<Image> OCR::charCrop(Image img){ 
...
char OCR::compareChar(Image croppedImage, std::vector<Image> iList) {
