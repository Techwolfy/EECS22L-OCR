...
std::vector<int> Image::findCropEdge() {
	...
	if(cropCoordinate[0] - offsetX - 1 >= 0){
		cropCoordinate[0] -= offsetX + 1;
	}else{
		cropCoordinate[0] = 0;
	}
	if(cropCoordinate[1] - offsetY - 1 >= 0){
		cropCoordinate[1] -= offsetY + 1;
	}else{
		cropCoordinate[1] = 0;
	}
	
	//adjust size fits multiples of char crop
	int tailX = (cropCoordinate[2] - cropCoordinate[0] + 1)% 30;
	int tailY = (cropCoordinate[3] - cropCoordinate[1] + 1)% 56;
	if( tailX != 0){
		if(tailX > 15 && cropCoordinate[2] + tailX <= w - 1){
			cropCoordinate[2] += 30 - tailX + 1;
		}else{
			cropCoordinate[2] -= tailX - 1;
		}
	}
	
	if( tailY != 0){
		if(tailY > 28 && cropCoordinate[3] + tailY <= h - 1){
			cropCoordinate[3] += 56 - tailY +1;
		}else{
			cropCoordinate[3] -= tailY - 1;
		}
	}
